...
from default import BACKTEST_AVERAGE_BASIS_POINTS
...
def run_single_window(
    ...
    train_interval: Tuple[int, int, int], #(2017,2021,2022)
    params: dict,
    ...
    raw_data = pd.read_csv(features_file_path, index_col=0, parse_dates=True) #读取cpd 数据
    ...
    results_sw = results_sw.merge(
        raw_data.reset_index()[["ticker", "date", "daily_vol"]].rename(
            columns={"ticker": "identifier", "date": "time"} #identifier is the asset
        ),
        on=["identifier", "time"],
    )
...
def run_all_windows(
    experiment_name: str,
    features_file_path: str,
    train_intervals: List[Tuple[int, int, int]], #[(2017,2021,2022),(2017,2022,2023)]
    params: dict,
    changepoint_lbws: List[int],
    asset_class_dictionary=Dict[str, str],
    hp_minibatch_size=HP_MINIBATCH_SIZE, # 干嘛
    ...
    for interval in train_intervals:
        run_single_window(
            experiment_name,
            features_file_path,
            interval, #(2017,2021,2022)
            params,
            changepoint_lbws,
            asset_class_dictionary=asset_class_dictionary,
            hp_minibatch_size=hp_minibatch_size,
        )
    print(experiment_name, train_intervals, asset_class_dictionary, standard_window_size)
