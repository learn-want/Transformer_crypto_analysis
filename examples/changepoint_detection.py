...
def fit_changepoint_kernel(
    ...
    m = gpflow.models.GPR(
        data=(
            time_series_data.loc[:, ["X"]].to_numpy(), #时间
            time_series_data.loc[:, ["Y"]].to_numpy(),  #return
        ),
    ...
    changepoint_location = m.kernel.locations[0].numpy() #change point 的位置
    params = {
...
def changepoint_loc_and_score(
    ...
    #add delete nan 2023.2.3 添加之后cpd 输出文件没有缺失值
    time_series_data=time_series_data[time_series_data["Y"].notna()]
    Y_data = time_series_data[["Y"]].values #daily return
    ...
    try:
        ...
        if kM_variance == kM_lengthscale == kM_likelihood_variance == 1.0:
            raise BaseException("Retry with default hyperparameters - already using default parameters.") from ex
    ...
    if is_cp_location_default:
        # default to midpoint # 如果取look back window 是 30，时间长度是 32，change
        #point 的初值是中点
        kC_changepoint_location = (
...
def run_module(
    time_series_data: pd.DataFrame, #传入的是整个价格时间序列
    ...
    if start_date and end_date:
        first_window = time_series_data.loc[:start_date].iloc[
            -(lookback_window_length + 1) :, :] #取开始值前面的 lbl
    ...
    for window_end in range(lookback_window_length + 1, len(time_series_data)):
        ...
        ][["date", "daily_returns"]].copy() #[0:30],[1:31],[2：32]
