...
class ModelFeatures:
    ...
    def _batch_data(self, data, sliding_window):
        ...
        id_col = get_single_col_by_input_type(InputTypes.ID, self._column_definition) #Returns name of single column.
        ...
        if sliding_window:
            ...
            def _batch_single_entity(input_data):
                ...
                lags = self.total_time_steps  # + int(self.extra_lookahead_steps) 参数lstm_time_steps
                x = input_data.values
                if time_steps >= lags:
                    return np.stack(
            ...
            for _, sliced in data.groupby(id_col):
                ...
                for k in col_mappings:
                    cols = col_mappings[k]
                    arr = _batch_single_entity(sliced[cols].copy()) #从sliced里面选取一列
                    print(arr)
